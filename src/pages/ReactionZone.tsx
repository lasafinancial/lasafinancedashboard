                            ...
                            <Table>
                                <TableHeader className="bg-white/[0.03]">
                                    <TableRow className="border-white/5 hover:bg-transparent">
                                        ...
                                        <TableHead className="text-[11px] font-black text-white/60 uppercase tracking-widest text-right cursor-pointer hover:text-white transition-colors" onClick={() => toggleSort("changePercent")}>
                                            Change % {sortField === "changePercent" && (sortDirection === "asc" ? <ChevronUp className="w-3 h-3 inline" /> : <ChevronDown className="w-3 h-3 inline" />)}
                                        </TableHead>
                                    ...
                                    <PremiumProtector requiredTier="pro" blurLevel="md" title="Premium Feature" description="Upgrade to view all Reaction Zone data.">
                                        {(isFree ? processedStocks.slice(0, 8) : processedStocks).map((stock) => (
                                            <TableRow key={stock.id} className="border-white/5 hover:bg-white/[0.04] transition-colors group">
                                                ...
                                                <TableCell className={`py-2 text-right font-bold tabular-nums text-xs ${(stock.changePercent ?? 0) >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
